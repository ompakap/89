...
class Imgs
{
	...
	public $thumb_square_size = 200; //Thumbnails will be cropped to 200x200 pixels
	public $max_image_size = 512; //Maximum image size (height and width)
	public $thumb_prefix = "thumb_"; //Normal thumb Prefix
	public $destination_folder = 'image_data/'; //upload directory ends with / (slash)
	public $jpeg_quality = 90; //jpeg quality
    ...
    public function processupload($id = '') 
	{
		...
		if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
		{
			...
			$image_name = $_FILES['fileInput']['name']; //file name
			$image_size = $_FILES['fileInput']['size']; //file size
			$image_temp = $_FILES['fileInput']['tmp_name']; //file temp
			...
			switch($image_type)
			{
				...
				case 'image/gif': $image_res =  imagecreatefromgif($image_temp); break;			
			...
			if($image_res)
			{
				...
				$new_file_name = 'vehi_' . $_POST["id"] . '.' . $image_extension;
				...
				if($this->normal_resize_image($image_res, $image_save_folder, $image_type, $this->max_image_size, $image_width, $image_height, $this->jpeg_quality))
				{
					...
					*/
					...
					echo '<img src="uploads/'.$this->thumb_prefix . $new_file_name.'" alt="Thumbnail">';
					echo '<br />';
